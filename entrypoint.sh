...
# Check if group with this GID exists, if so use existing group name
if existing_group=$(getent group "$GROUP_ID" | cut -d: -f1); then
...
if ! id -u "$USER_ID" > /dev/null 2>&1; then
    ...
    # Note: This gives full sudo access for development convenience
