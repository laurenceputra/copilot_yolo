...
categorize_commit() {
  local subject="$1" paths="$2" lc_subject subject_keyword path_category conventional_prefix_regex
  lc_subject="$(printf '%s' "$subject" | tr '[:upper:]' '[:lower:]')"
  conventional_prefix_regex='^([a-z][a-z0-9_-]*)(\([^)]+\))?(!)?:'
  if [[ "$lc_subject" =~ $conventional_prefix_regex ]]; then
    subject_keyword="${BASH_REMATCH[1]}"
  else
    subject_keyword="${lc_subject%%[[:space:]]*}"
  fi

  if [[ "$subject_keyword" =~ ^(feat|feature|add|adds|added|introduce|new)$ ]]; then
    printf 'Added\n'
    return 0
  fi
  if [[ "$subject_keyword" =~ ^(fix|fixed|bugfix|hotfix|patch|resolve|resolved|repair)$ ]]; then
    printf 'Fixed\n'
    return 0
  fi
  if [[ "$subject_keyword" =~ ^(docs|doc|documentation|readme|changelog|contributing|technical)$ ]]; then
    printf 'Documentation\n'
    return 0
  fi
  if [[ "$subject_keyword" =~ ^(ci|build|workflow|actions)$ ]]; then
    printf 'CI\n'
    return 0
  fi
...
while IFS=$'\t' read -r sha subject || [[ -n "${sha:-}" ]]; do
  ...
  case "$category" in
    Added)
      printf '%s%s%s\n' "$subject" "$section_fs" "$short_sha" >> "$added_file"
      ;;
    Fixed)
      printf '%s%s%s\n' "$subject" "$section_fs" "$short_sha" >> "$fixed_file"
      ;;
    Documentation)
      printf '%s%s%s\n' "$subject" "$section_fs" "$short_sha" >> "$docs_file"
      ;;
    CI)
      printf '%s%s%s\n' "$subject" "$section_fs" "$short_sha" >> "$ci_file"
      ;;
    *)
      printf '%s%s%s\n' "$subject" "$section_fs" "$short_sha" >> "$changed_file"
      ;;
  esac
