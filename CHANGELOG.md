...
- Enabled BuildKit cache mounts for `apt` and `npm` in `.copilot_yolo.Dockerfile`, while temporarily disabling Debian's default `docker-clean` behavior only during the cache-using build step
